  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <Card>
            ...
            {entries.map((entry, index) => (
              ...
              >
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {entries.length > 1 && (
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-50 sm:hidden"
                ...
                {entryErrors[entry.id] && (
                  <p className="col-span-1 sm:col-span-12 text-red-500 text-xs mt-1">
                    {entryErrors[entry.id]}
                  </p>
                )}
              </div>
        ...
        <CardFooter className="flex flex-col sm:flex-row justify-between items-center border-t bg-slate-50 gap-3">
          <div className="text-sm text-slate-500 w-full sm:w-auto text-center sm:text-left mb-2 sm:mb-0">
            {timesheetExists ? 'Update your timesheet when ready' : 'Submit your timesheet when ready'}
          </div>
          <Button
            variant="default"
            className="bg-indigo-600 hover:bg-indigo-700 w-full sm:w-auto"
            onClick={submitTimesheet}
            disabled={totalError || isSaving || Object.keys(entryErrors).length > 0}
          >
            {isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            {timesheetExists ? 'Update Timesheet' : 'Submit Timesheet'}
          </Button>
        </CardFooter>
      </Card>
    </div>
